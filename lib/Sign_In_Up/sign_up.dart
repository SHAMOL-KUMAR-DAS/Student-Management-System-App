    ...
    if(cformkey.validate()){
      ...
      try {
        if (RegExp(r'^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[!@#\$&*~]).{6,}$').hasMatch(password.text) && password.text == repassword.text) {
          ...
          Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=>OTP_Verification(_emailController.text, widget.type)));
        }
      }
      catch(e){
        print(e.message);
        ...
        body: Container(
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                //Image
                Image(image: AssetImage('assets/images/teacher_in.png'),height: 250,),
